#!/bin/bash
# Deploys a new erp-cbqa-global binary to the PRODUCTION box for this API
# (72.60.74.36, api-pm-prod.cbqaglobal.co.id): backup current binary, apply
# any new migration SQL (idempotent, IF NOT EXISTS-guarded), swap the
# binary, restart the systemd service, health-check it, auto-rollback on
# failure.
#
# Usage: deploy-golang-prod.sh <path-to-extracted-artifact-dir>
#   (that dir must contain ./erp-cbqa-global and optionally ./migrate/*.sql)
#
# Runs on the server itself, invoked over SSH by
# .github/workflows/deploy-production.yml (push-based — see
# ops/README.md for why this differs from the older pull-based setup
# targeting 72.60.74.35).

set -euo pipefail

SRC_DIR="${1:?Usage: deploy-golang-prod.sh <extracted-artifact-dir>}"
APP_DIR="/root/app/golang-pm-prod"
BIN_PATH="$APP_DIR/erp-cbqa-global"
BACKUP_DIR="$APP_DIR/backups"
SERVICE_NAME="cbqaglobal-golang-pm-prod"
