#!/bin/bash
# Deploys a new erp-cbqa-global binary: backup current binary, apply any new
# migration SQL (idempotent, IF NOT EXISTS-guarded), swap the binary,
# restart the systemd service, health-check it, auto-rollback on failure.
#
# Usage: deploy-golang.sh <path-to-extracted-artifact-dir>
#   (that dir must contain ./erp-cbqa-global and optionally ./migrate/*.sql)
#
# Mirrors CBQAGLOBAL-CRM-BACKEND's /root/app/backend/deploy.sh.

set -euo pipefail

SRC_DIR="${1:?Usage: deploy-golang.sh <extracted-artifact-dir>}"
APP_DIR="/root/app/golang-pm"
BIN_PATH="$APP_DIR/erp-cbqa-global"
BACKUP_DIR="$APP_DIR/backups"
SERVICE_NAME="cbqaglobal-golang-pm"
